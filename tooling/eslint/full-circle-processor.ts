...
/**
 * Lets the standard React Hooks rules recognize a component's `state`
 * callback as a hook without changing authored code or source positions.
 */
export const fullCircleProcessor = {
  meta: {
    name: "full-circle/react-hooks",
    version: "0.1.0",
  },
...
function isCreateComponentDefinition(node: ts.ObjectLiteralExpression) {
  ...
  return (
    ...
    (ts.isPropertyAccessExpression(factory) && factory.name.text === "createComponent")
  );
}
