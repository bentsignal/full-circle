...
describe("hot runtime reloads", () => {
  test("updates mounted components across a fresh runtime module instance", async () => {
    ...
    const FirstStore = firstRuntime.createStore<{ value: string }>().__fullCircleHot(storeId);
    ...
    act(() => {
      RefreshedStore = refreshedRuntime.createStore<{ value: string }>().__fullCircleHot(storeId);
      RefreshedComponent = refreshedRuntime
        .createComponent({
          deps: [RefreshedStore],
          state: ({ deps }) => ({
            value: refreshedRuntime.useStore(deps.store!, (state) => state.value),
          }),
