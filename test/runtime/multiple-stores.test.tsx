...
describe("multiple store providers", () => {
  test("updates three independent stores across nested provider boundaries", () => {
    ...
    expect(screen.getByLabelText("Transformed operator")).toHaveTextContent("ADA");
    ...
    expect(screen.getByLabelText("Transformed operator")).toHaveTextContent("GRACE HOPPER");
    ...
    fireEvent.click(screen.getByRole("button", { name: "Increase build count" }));
    fireEvent.click(screen.getByRole("button", { name: "Increase build count" }));
