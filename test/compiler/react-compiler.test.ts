...
import { lowerFullCircleSources } from "../../src/compiler";
...
describe("React Compiler lowering", () => {
  it("compiles a stateless component without manufacturing state", async () => {
    ...
    expect(result.events.filter((event) => event.kind === "CompileSuccess")).toHaveLength(1);
  });
  ...
  it("compiles component state, ui, and provider hooks", async () => {
    ...
    expect(result.lowered).toMatch(/state: \(\{ deps \}\) => \{\s*"use memo";/u);
    expect(result.lowered).toMatch(/ui: \(\{ state \}\) => \{\s*"use memo";/u);
    expect(result.lowered).toContain('deps: [Counter.__fullCircleDependency("counter")]');
    expect(result.lowered).toContain(
      'createStore<{ count: number }>().__fullCircleNamed("Counter").__fullCircleHot("store:/project/counter.tsx#Counter")',
    );
    expect(result.lowered).toMatch(
      /\.__fullCircleNamed\("CounterValue"\)\.__fullCircleHot\("component:\/project\/counter\.tsx#CounterValue", \{"state":"[a-f0-9]{16}","ui":"[a-f0-9]{16}"\}\)/u,
    );
    expect(result.lowered).toContain(".__fullCircleProvidedRequirements([Counter], CounterValue)");
    expect(result.lowered).toContain(
      "implements={Counter.__fullCircleImplementation(useCounterImplementation())}",
    );
    expect(result.code).toContain('from "react/compiler-runtime"');
    expect(result.code.match(/\b_c\(/gu)).toHaveLength(4);
    expect(result.events.filter((event) => event.kind === "CompileSuccess")).toHaveLength(4);
    expect(
      result.events.some(
        (event) =>
    ...
    expect(
      result.events.some(
        (event) => event.kind === "CompileError" || event.kind === "PipelineError",
      ),
    ).toBe(false);
  });
  ...
  it("compiles a named state callback without hook naming conventions", async () => {
    const source = `
      import { useState } from "react";
      import { createComponent, createStore } from "full-circle";
    ...
    expect(result.lowered).toContain('function formState({ deps }) {\n"use memo";');
    expect(result.code).toContain("_c(");
    expect(
      result.events.some(
        (event) =>
          event.kind === "CompileSuccess" && event.fnName === "formState" && event.memoSlots > 0,
      ),
    ).toBe(true);
  });
});
