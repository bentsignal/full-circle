...
import { analyzeFullCircle, lowerFullCircleSources } from "../../src/compiler";

describe("analyzeFullCircle", () => {
  it("propagates nested JSX requirements and subtracts a root provider", () => {
    const analysis = analyzeFullCircle({
      sources: [
        {
  ...
  it("subtracts a provider halfway through a nested graph", () => {
    ...
    expect(analysis.diagnostics[0]?.message).toContain("Theme via Panel -> Leaf");
  });
  ...
  it("resolves aliased imports and requirements across files", () => {
      ...
      sources: [
        {
          ...
          source: `
            import { createStore } from "full-circle";
    ...
    expect(analysis.diagnostics[0]?.message).toContain("Auth via Panel -> Button");
    expect(
...
describe("lowerFullCircleSources", () => {
      ...
      expect.arrayContaining([
        expect.objectContaining({
          text: expect.stringMatching(
            /^\.__fullCircleNamed\("Parent"\)\.__fullCircleHot\("component:\/project\/example\.tsx#Parent", \{"state":"[a-f0-9]{16}","ui":"[a-f0-9]{16}"\}\)\.__fullCircleRequirements\(Child\)\.__fullCircleAnalyzed\(\)$/u,
          ),
        }),
      ]),
    );
  ...
  it("signs resolved named state and ui callbacks independently", () => {
    ...
    const signature = /__fullCircleHot\([^,]+, (\{[^)]+\})\)/u;
    const beforeSignatures = JSON.parse(before.match(signature)?.[1] ?? "{}") as {
      state?: string;
      ui?: string;
    };
  ...
  it("lowers provider subtrees into requirement subtraction", () => {
    ...
    expect(lowered?.source).toContain(".__fullCircleProvidedRequirements([Counter], Child)");
  });
});

function requirementsOf(analysis: ReturnType<typeof analyzeFullCircle>, name: string) {
  return analysis.components.find((component) => component.name === name)?.requirements;
}
