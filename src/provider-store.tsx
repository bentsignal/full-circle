...
> extends Context.Service<StoreRequirement<Name, State>, ReadableStore<State>> {
  ...
  readonly __fullCircleDependencyKey: Key;
...
export interface Store<Name extends string, State extends object> extends StoreDependency<
  Uncapitalize<Name>,
  Name,
  State
> {
  (props: StoreProps<State>): ReactNode;
  /** @internal Used only by the in-memory Full Circle transform. */
  readonly __fullCircleDependency: <const Key extends string>(
    key: Key,
  ) => StoreDependency<Key, Name, State>;
  /** @internal Used only by the in-memory Full Circle transform. */
  readonly __fullCircleImplementation: (state: State) => StoreImplementation<State>;
  /** @internal Used only by the in-memory Full Circle transform. */
  readonly __fullCircleHot: (id: string) => Store<Name, State>;
  /** @internal Used only by the in-memory Full Circle transform. */
  readonly __fullCircleNamed: <const InferredName extends string>(
    name: InferredName,
...
export function createStore<State extends object>() {
  ...
  const dependency = Context.Service<StoreRequirement<string, State>, ReadableStore<State>>(
    `full-circle/store/${identity}`,
  );
  ...
  function Store({ children, implements: implementation }: StoreProps<State>) {
    ...
    return <ServiceContextProvider services={services}>{children}</ServiceContextProvider>;
  }

  Object.setPrototypeOf(Store, Object.getPrototypeOf(dependency));
  Object.assign(Store, dependency, {
    __fullCircleDependency: (key: string) =>
      Object.assign(Object.create(dependency) as object, {
        __fullCircleDependencyKey: key,
      }),
    __fullCircleDependencyKey: "store",
    __fullCircleImplementation: (state: State) => state,
    __fullCircleHot: (id: string) => registerHotStore(id, Store),
    __fullCircleNamed: (name: string) => {
      Object.assign(Store, {
        __fullCircleDependencyKey: `${name.slice(0, 1).toLowerCase()}${name.slice(1)}`,
