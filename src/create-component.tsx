...
import type { StoreDependency, StoreRequirement, StoreTypeId } from "./provider-store";
...
export declare const FullCircleAnalysisRequiredTypeId: unique symbol;
...
export const defineProps = <Props,>() => componentProps as ComponentProps<Props>;

export interface FullCircleAnalysisRequired {
  readonly [FullCircleAnalysisRequiredTypeId]: "Full Circle compiler analysis is not active";
}
...
type RequirementServices<Requirements> =
  Requirements extends ComponentRequirements<infer Services> ? Services : Requirements;
...
> {
  ...
  readonly __fullCircleAnalyzed: () => CreatedComponent<
    Props,
    Exclude<RequirementServices<Requirements>, FullCircleAnalysisRequired>
  >;
  readonly __fullCircleRequirements: <const Components extends readonly unknown[]>(
    ...components: Components
  ) => CreatedComponent<
    Props,
    RequirementServices<Requirements> | RequirementsOf<Components[number]>
  >;
  readonly __fullCircleNamed: (name: string) => CreatedComponent<Props, Requirements>;
  readonly __fullCircleHot: (
    id: string,
    signatures: HotComponentSignatures,
  ) => CreatedComponent<Props, Requirements>;
  readonly __fullCircleProvidedRequirements: <
  ...
  >(
    ...
    Props,
    | RequirementServices<Requirements>
    | Exclude<RequirementsOf<Components[number]>, ProviderRequirements<Providers[number]>>
  >;
}

export interface Component<
  Requirements extends ComponentRequirements<unknown> = ComponentRequirements<never>,
...
export interface ComponentWithProps<
  Props,
  Requirements extends ComponentRequirements<unknown> = ComponentRequirements<never>,
> extends ComponentProtocol<Props, Requirements, ComponentWithProps<Props, Requirements>> {
...
type CreatedComponent<Props, Requirements> =
  IsEmptyProps<Props> extends true
    ...
    : ComponentWithProps<Props, ComponentRequirements<RequirementServices<Requirements>>>;
...
export type ComponentEffect<Value> = Effect.Effect<RenderResult, never, RequirementsOf<Value>>;
...
type DependencyRequirements<Dependencies extends StoreDependencies> = DependencyRequirement<
  Dependencies[number]
>;

export type ComponentState<Props, Dependencies extends StoreDependencies, State> = (input: {
...
export type StatefulComponentDefinition<Props, Dependencies extends StoreDependencies, State> = {
...
type StatefulComponentWithoutPropsDefinition<Dependencies extends StoreDependencies, State> = {
...
export type StatelessComponentDefinition<Dependencies extends StoreDependencies> = {
  readonly props?: never;
  readonly deps: readonly [...Dependencies];
...
export function createComponent<Props, const Dependencies extends StoreDependencies, State>(
  definition: StatefulComponentDefinition<Props, Dependencies, State>,
): CreatedComponent<Props, DependencyRequirements<Dependencies> | FullCircleAnalysisRequired>;
export function createComponent<const Dependencies extends StoreDependencies, State>(
  definition: StatefulComponentWithoutPropsDefinition<Dependencies, State>,
): CreatedComponent<EmptyProps, DependencyRequirements<Dependencies> | FullCircleAnalysisRequired>;
...
export function createComponent(definition: unknown) {
  ...
  const CreatedComponent = (props: object) => {
    ...
    const dependencies = resolveDependencies(componentDefinition.deps ?? [], services);
  ...
  return eraseComponentType(makeComponent<object, never>(CreatedComponent, hotState));
}
...
function makeComponent<Props, Requirements>(
  ...
  >({
    label: "FullCircleComponent",
    evaluate: () => Effect.context<Requirements>().pipe(Effect.as(created)),
  });
  Object.assign(created, effectPrototype, {
    __fullCircleAnalyzed: () => created,
    __fullCircleHot: (id: string, signatures: HotComponentSignatures) =>
    ...
    __fullCircleNamed: (name: string) => {
      Object.assign(component, { displayName: name });
      return created;
    },
    __fullCircleProvidedRequirements: () => created,
    __fullCircleRequirements: () => created,
  });
  return created;
}
...
function EvaluatedUI({ state, ui }: { state?: unknown; ui: RuntimeComponentDefinition["ui"] }) {
  "use no memo";
...
function resolveDependencies(
  ...
  if (Exit.isSuccess(exit)) {
    const entries = dependencies.map((dependency, index) => {
      ...
      return [dependency.__fullCircleDependencyKey, store] as const;
    });
...
type ResolvedRuntimeDependencies = Readonly<Record<string, ReadableStore<object>>>;
