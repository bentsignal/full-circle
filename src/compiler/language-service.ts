...
import type { LoweredFullCircleSource, SourceInsertion } from "./lowering.js";
import {
...
export function createFullCircleLanguageService({
  languageService,
  languageServiceHost,
  typescript,
}: {
  ...
  const loweredHost = new Proxy(languageServiceHost, {
    get(target, property) {
      ...
      if (property === "getScriptVersion") {
        return (fileName: string) =>
          loweredProject.scriptVersion(fileName) ?? target.getScriptVersion(fileName);
      }
  ...
  const overrides = {
    getQuickInfoAtPosition(fileName, position, maximumLength) {
      ...
      const quickInfo = loweredService.getQuickInfoAtPosition(
        fileName,
        lowered ? originalToLoweredPosition(position, lowered.insertions) : position,
        maximumLength,
      );
      return quickInfo && lowered
        ? {
            ...quickInfo,
            textSpan: mapTextSpanToOriginal(quickInfo.textSpan, lowered.insertions),
          }
        : quickInfo;
    },
...
function createLoweredProject({
  ...
  let cacheKey = "";
  let lowered = new Map<string, LoweredFullCircleSource>();
  let scriptVersions = new Map<string, number>();

  const refresh = () => {
    const fileNames = host.getScriptFileNames().filter((fileName) => isFullCircleSource(fileName));
    ...
    const nextLowered = lowerFullCircleSources(
    ...
    for (const [fileName, source] of nextLowered) {
      ...
      nextScriptVersions.set(
        fileName,
        previous?.source === source.source ? previousVersion : previousVersion + 1,
      );
    }
  ...
  return {
    ...
    getOriginalSourceFile(fileName: string) {
      ...
      return typescript.createSourceFile(
        ...
        true,
        fileName.endsWith("x") ? typescript.ScriptKind.TSX : typescript.ScriptKind.TS,
      );
    },
...
function mapDiagnosticToOriginal({
  ...
  const end =
    diagnostic.start === undefined
      ? undefined
      : loweredToOriginalPosition(diagnostic.start + (diagnostic.length ?? 0), lowered.insertions);
  const file =
  ...
  return {
    ...diagnostic,
    file,
    length: start === undefined || end === undefined ? diagnostic.length : end - start,
...
function mapTextSpanToOriginal(textSpan: ts.TextSpan, insertions: readonly SourceInsertion[]) {
  const start = loweredToOriginalPosition(textSpan.start, insertions);
  const end = loweredToOriginalPosition(textSpan.start + textSpan.length, insertions);
  return { length: end - start, start };
}

function isFullCircleSource(fileName: string) {
