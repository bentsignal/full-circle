...
import { buildComponentGraph, computeRequirements, findCycles } from "./graph.js";
import { buildSourceModel } from "./source-model.js";

export function analyzeFullCircle(options: AnalyzeFullCircleOptions) {
  const normalizedSources = new Map(
    options.sources.map((source) => [normalizeFileName(source.fileName), source.source]),
  );
...
function toAnalyzedComponent({
  ...
  return {
    dependencies: sorted(component.dependencies.map((dependency) => dependency.id)),
...
function compareDiagnostics(left: FullCircleDiagnostic, right: FullCircleDiagnostic) {
  return (
