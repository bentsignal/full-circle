...
import type { FullCircleSource } from "./types.js";
...
export interface LoweredFullCircleSource {
  readonly insertions: readonly SourceInsertion[];
  readonly source: string;
}

export function lowerFullCircleSources(sources: readonly FullCircleSource[]) {
  const normalizedSources = new Map(
    sources.map((source) => [normalizeFileName(source.fileName), source.source]),
  );
  const models = new Map<string, SourceModel>();

  for (const [fileName, source] of normalizedSources) {
    models.set(fileName, buildSourceModel({ fileName, knownFiles: normalizedSources, source }));
  }
  ...
  return new Map(
    [...models].map(([fileName, model]) => {
      ...
      return [
        ...
        {
          insertions,
          source: applyInsertions(normalizedSources.get(fileName) ?? "", insertions),
        } satisfies LoweredFullCircleSource,
      ];
    }),
  );
}
...
function collectInsertions({
  ...
  for (const declaration of model.components.values()) {
    const groups = new Map<string, (typeof declaration.jsxChildReferences)[number][]>();
    for (const reference of declaration.jsxChildReferences) {
      if (!resolveComponent({ models, reference: reference.component })) {
        continue;
      }
      const key = reference.providers.map((provider) => provider.name).join("\u0000");
      const group = groups.get(key) ?? [];
      group.push(reference);
      groups.set(key, group);
    }
    const annotations = Array<string>(
      `.__fullCircleNamed(${JSON.stringify(declaration.name)})`,
      `.__fullCircleHot(${JSON.stringify(hotIdentity("component", model.fileName, declaration.name))}, ${JSON.stringify(declaration.hotSignatures)})`,
    );

    for (const references of groups.values()) {
      const children = [...new Set(references.map((reference) => reference.component.name))];
      const providers = [
        ...new Set(
          references.flatMap((reference) => reference.providers.map((provider) => provider.name)),
        ),
      ];

      annotations.push(
        providers.length === 0
          ? `.__fullCircleRequirements(${children.join(", ")})`
          : `.__fullCircleProvidedRequirements([${providers.join(", ")}], ${children.join(", ")})`,
      );
    }

    annotations.push(".__fullCircleAnalyzed()");
    insertions.push({
...
function collectStoreNameInsertions(model: SourceModel) {
  return [...model.stores.values()].map((store) => ({
    ...
    text: `.__fullCircleNamed(${JSON.stringify(store.name)}).__fullCircleHot(${JSON.stringify(hotIdentity("store", model.fileName, store.name))})`,
  }));
}

function hotIdentity(kind: "component" | "store", fileName: string, name: string) {
...
function collectStoreDependencyInsertions({
  ...
  const dependencyLocations = new Set(
    [...model.components.values()].flatMap((component) =>
      component.storeReferences.map(
        (reference) => `${reference.location.line}:${reference.location.column}:${reference.name}`,
      ),
    ),
  );
  ...
  visit(model.sourceFile, (node) => {
    ...
    if (!dependencyLocations.has(locationKey) || !resolveStore({ models, reference })) {
      return;
    }
    ...
    insertions.push({
      position: node.end,
      text: `.__fullCircleDependency(${JSON.stringify(dependencyKey)})`,
    });
  });
  ...
  visit(model.sourceFile, (node) => {
    ...
    for (const property of node.attributes.properties) {
      ...
      insertions.push({
        ...
        text: `${node.tagName.text}.__fullCircleImplementation(`,
      });
      insertions.push({
...
function applyInsertions(source: string, insertions: readonly SourceInsertion[]) {
  let lowered = source;

  for (const insertion of [...insertions].reverse()) {
    lowered =
      lowered.slice(0, insertion.position) + insertion.text + lowered.slice(insertion.position);
  }
