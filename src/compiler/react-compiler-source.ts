...
const fullCircleModule = "full-circle";
...
export function collectCompilerImports(sourceFile: ts.SourceFile) {
  ...
  for (const statement of sourceFile.statements) {
    ...
    if (moduleName === fullCircleModule) {
      collectFullCircleBindings({
        apiNamespaces,
        bindings,
        componentFactories,
      });
...
export function findPropertyExpression(definition: ts.ObjectLiteralExpression, name: string) {
...
function collectFullCircleBindings({
  apiNamespaces,
  bindings,
  componentFactories,
}: {
  ...
  for (const element of bindings.elements) {
    if ((element.propertyName?.text ?? element.name.text) === "createComponent") {
