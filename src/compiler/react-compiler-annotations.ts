...
import { collectModuleCallbacks, propertyName } from "./react-compiler-source.js";
...
export function annotateExpression(context: AnnotationContext, expression: ts.Expression) {
...
function annotateFunction(context: AnnotationContext, callback: ts.FunctionLikeDeclaration) {
...
function hasMemoDirective(body: ts.Block) {
  for (const statement of body.statements) {
    ...
    if (statement.expression.text === "use memo" || statement.expression.text === "use no memo") {
      return true;
    }
  }
  return false;
