...
import type { FullCircleDiagnostic } from "./types.js";
...
export function buildComponentGraph({
  ...
  readonly diagnostics: FullCircleDiagnostic[];
...
export function computeRequirements(graph: ReadonlyMap<string, GraphComponent>) {
...
export function findCycles(graph: ReadonlyMap<string, GraphComponent>) {
  const diagnostics = Array<FullCircleDiagnostic>();
...
export function unresolvedReferenceDiagnostic(reference: SymbolReference, kind: string) {
  return {
    code: "unresolved-analysis-reference",
    fileName: reference.fileName,
    location: reference.location,
    message: `Unable to resolve ${kind} "${reference.name}" while analyzing Full Circle requirements.`,
  } satisfies FullCircleDiagnostic;
}
...
function collectDirectRequirements({
  ...
  for (const reference of component.declaration.storeReferences) {
    ...
    if (store) {
      ...
      diagnostics.push(unresolvedReferenceDiagnostic(reference, "store requirement"));
    }
  }
}
    ...
    if (child) {
      ...
      for (const providerReference of reference.providers) {
        ...
        if (store) {
          ...
          diagnostics.push(unresolvedReferenceDiagnostic(providerReference, "provider store"));
        }
      }
...
function reportCycle({
  ...
  diagnostics.push({
    ...
    message: `Full Circle component cycle detected: ${cycle
      .map((componentId) => graph.get(componentId)?.declaration.name ?? componentId)
      .join(" -> ")}.`,
  });
}
...
function resolveDeclaration<Declaration>({
  ...
  readonly getDeclaration: (model: SourceModel, name: string) => Declaration | undefined;
  readonly models: ReadonlyMap<string, SourceModel>;
  readonly reference: SymbolReference;
}) {
