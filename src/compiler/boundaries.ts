...
import type { AnalyzeFullCircleOptions, FullCircleDiagnostic, ReactBoundary } from "./types.js";
...
export function buildExplicitBoundaries({
  ...
  readonly models: ReadonlyMap<string, SourceModel>;
  readonly requirements: ReadonlyMap<string, ReadonlySet<string>>;
  readonly roots: AnalyzeFullCircleOptions["roots"];
}) {
  const boundaries = Array<ReactBoundary>();
  ...
  for (const root of roots ?? []) {
    ...
    if (!component) {
      diagnostics.push(unresolvedReferenceDiagnostic(reference, "root component"));
      continue;
    }
...
export function addUnresolvedBoundaryDiagnostics({
  ...
  for (const boundary of boundaries) {
    ...
    diagnostics.push({
      ...
      message: `${boundary.kind === "react" ? `React component "${boundary.ownerName}" renders` : `Root "${boundary.ownerName}" resolves to`} Full Circle component "${boundary.componentName}" with unresolved stores: ${boundary.requirements
        .map(
          (requirement) =>
...
function findRequirementPath({
  ...
  while (queue.length > 0) {
    ...
    for (const dependency of component.dependencies) {
      ...
      queue.push({
        id: dependency.id,
        path: [...current.path, graph.get(dependency.id)?.declaration.name ?? dependency.id],
      });
    }
  }
