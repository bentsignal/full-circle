...
export function findPropertyInitializer(object: ts.ObjectLiteralExpression, name: string) {
...
export function resolveModuleFile({
  ...
  const base = normalizeFileName(path.resolve(path.dirname(fileName), moduleName));
  const candidates = [
    base,
    ...sourceExtensions.map((extension) => `${base}${extension}`),
    ...sourceExtensions.map((extension) => normalizeFileName(path.join(base, `index${extension}`))),
  ];
