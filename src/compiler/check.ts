...
import type { LoweredFullCircleSource } from "./lowering.js";
import { loweredToOriginalPosition, lowerFullCircleSources } from "./lowering.js";
...
if (config.error) {
  ...
  const sourceFiles = authoredProgram
    ...
    .filter(
      (fileName) =>
        !fileName.endsWith(".d.ts") && !fileName.includes(`${path.sep}node_modules${path.sep}`),
    );
  ...
  const diagnostics = [...parsed.errors, ...ts.getPreEmitDiagnostics(program)].map((diagnostic) =>
    mapDiagnosticToOriginal(diagnostic, lowered),
  );
...
function createLoweredCompilerHost(
  ...
  lowered: ReadonlyMap<string, LoweredFullCircleSource>,
) {
...
function findConfigFileName(arguments_: readonly string[]) {
  ...
  const projectArgument = projectIndex === -1 ? undefined : arguments_[projectIndex + 1];
...
function mapDiagnosticToOriginal(
  ...
  const start = loweredToOriginalPosition(diagnostic.start, loweredSource.insertions);
  ...
  return {
    ...
    file: source
      ? ts.createSourceFile(
          ...
          true,
          diagnostic.file.fileName.endsWith("x") ? ts.ScriptKind.TSX : ts.ScriptKind.TS,
        )
      : diagnostic.file,
    length: end - start,
    start,
